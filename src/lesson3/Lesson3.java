...
public class Lesson3 {
   ...
   */
	static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
		...
		// My solution is sub optimal, I should used for each and a for inside it
		for (int i = 0; i < LIST_SIZE; i++) {
			IntStream intStream = IntStream.range(0, LIST_SIZE);
			if (parallel) {
				intStream = intStream.parallel();
			}
			final String sourceWord = wordList.get(i);
			distances[i] = intStream.map(j -> Levenshtein.lev(sourceWord, wordList.get(j))).toArray();
		}
		return distances;
	}
